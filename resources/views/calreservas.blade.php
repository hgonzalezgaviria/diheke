            ...
            var html = '<div class="col-xs-10 col-sm-3 col-md-3 col-lg-3 zoom-in-hover">'+
                          '<div class="panel panel-default">'+
                            '<div align="letf" class="panel-heading">'+salas[i].SALA_DESCRIPCION+
                              ...
                              '{{ Form::open( ['url' => 'reservas/calreservas', 'method' => 'get', 'class'=>'form-vertical' ]  ) }}'+
                              ' <input name="sala" type="hidden" value='+salas[i].SALA_ID+'>'+
                              '{{ Form::button('<i class="fa fa-ticket" aria-hidden="true"></i> Ver Reservas', [ 'class'=>'btn-xs btn-primary btn-xs', 'type'=>'submit' ]) }}'+
                              '{{ Form::close() }}'+
                            '</div>'+
                          '</div>'+
                        '</div>';
...
@section('content')
<h1 class="page-header">Calendario de Reservas</h1>
